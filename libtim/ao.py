...
def inspect_influence(inflact, inflmeas, apt_mask, infldat=None, what="all", fignum0=1000, store=False, outdir='./'):
	...
	if (what in ["all", "actrec"]):
		...
		vmin, vmax = inflact.min(), inflact.max()
		plt.subplot(131)
		plt.xlabel("Actuator [id]")
		plt.ylabel("Measurement [#]")
		plt.imshow(inflact, vmin=vmin, vmax=vmax)
		plt.subplot(132)
		plt.xlabel("Actuator [id]")
		plt.imshow(infldat['actmat_rec'], vmin=vmin, vmax=vmax)
		plt.colorbar()
		plt.subplot(133)
		plt.xlabel("Actuator [id]")
		plt.imshow(inflact[:,:-1]-infldat['actmat_rec'])
		plt.colorbar()
